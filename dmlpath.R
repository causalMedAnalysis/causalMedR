...
#'   is given by \( \text{PSE}_{D \to Y}(d, d^*) = \text{NDE}_{M}(d, d^*) \).
#' 
#' - For each intermediate mediator \( M_k \), the path-specific effect is computed 
#'   as the difference between successive NDEs:
#'   \[
#'   \text{PSE}_{D \to M_k \rightsquigarrow Y}(d, d^*) = \text{NDE}_{M_{k-1}}(d, d^*) - \text{NDE}_{M_k}(d, d^*).
#'   \]
#' 
#' - The first path-specific effect (from \( D \to M_1 \rightsquigarrow Y \)) is equal 
#'   to the natural indirect effect for \( M_1 \), i.e.:
#'   \[
...
dmlpath <- function(
  ...
  for(k in rev(seq_len(K))){
    ...
    if(K == 1) {
      PSE[[K]] <- est[["est2"]] %>% dplyr::filter(Estimand == "NDE(1,0)") %>% dplyr::pull(Mean)
      PSE[[K + 1]] <- est[["est2"]] %>% dplyr::filter(Estimand == "NIE(1,0)") %>% dplyr::pull(Mean)
      SE[[K]] <- est[["est2"]] %>% dplyr::filter(Estimand == "NDE(1,0)") %>% dplyr::pull(SE)
      SE[[K + 1]] <- est[["est2"]] %>% dplyr::filter(Estimand == "NIE(1,0)") %>% dplyr::pull(SE)
      ATE <- est[["est2"]] %>% dplyr::filter(Estimand == "ATE(1,0)")
